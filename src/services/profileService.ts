...
const apiRoute: string = 'http://localhost:5169/api/person';
...
class PersonService {
  ...
  async list(prefix: string, universityFilter?: string): Promise<PersonSearchModel[]> {
    let response = await fetch(`${apiRoute}?prefix=${encodeURIComponent(prefix)}&university=${encodeURIComponent(universityFilter || '')}`);
...
export const profileService = new PersonService();
