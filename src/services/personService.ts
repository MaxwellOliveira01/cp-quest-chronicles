...
class PersonService {
  ...
  async list(prefix: string, universityFilter?: string): Promise<PersonSearchModel[]> {
    ...
    // Fix university filter to properly filter at database level
    if (universityFilter) {
      ...
      if (universityData) {
        ...
        // If university doesn't exist, return empty array
        return [];
      }
    }
  ...
  async create(person: Omit<PersonFullModel, 'id' | 'teams' | 'events' | 'contests'>): Promise<void> {
  ...
  async update(id: string, person: Omit<PersonFullModel, 'id' | 'teams' | 'events' | 'contests'>): Promise<void> {
    // Find university ID if university name is provided
    let universityId = null;
    if (person.university) {
      const { data: university } = await supabase
